...
public:
	...
	void load_obj(std::string obj_path)
	{
		...
		if (suffix == "obj") {
			 ...
			 */
			for (Face_Index f : indexed_faces)
			{
				for (int i =0; i < 3; i++)
				{
					glm::vec3 tempPos = ori_positions.at(f.vertex[i].pos_idx);
					glm::vec3 tempNorm = ori_normals.at(f.vertex[i].normal_idx);
					glm::vec2 tempTex = ori_texcoords.at(f.vertex[i].texcoord_idx);
					Vertex v;
					v.Position = tempPos;
					v.Normal = tempNorm;
					v.TexCoords = tempTex;
